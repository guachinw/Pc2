...
{
	static public Vector3 Seek(SBAgent agent, Transform target, float range = 99999) //buscar al target
	{
	...
	static public Vector3 Flee(SBAgent agent, Transform target, float range = 99999) //Huir del Target
	{
		// cálculo del vector deseado
		Vector3 desired = Vector3.zero;
		Vector3 difference = -(target.position - agent.transform.position);
	...
	static public Vector3 Arrive(SBAgent agent, Transform target, float range) //llegada
	{
		// cálculo del vector deseado
		Vector3 desired;
		Vector3 difference = (target.position - agent.transform.position);
